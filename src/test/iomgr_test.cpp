...
void IOMgrTester::start() {
    ...
    for (size_t i = 0; i < num_ev_fd; i++) {
        ...
        m_iomgr->add_fd(m_ev_fd[i], [this](auto fd, auto cookie, auto event) { process_ev_callback(fd, cookie, event); },
           EPOLLIN, 9, nullptr);
    }
    ...
    if (num_attemps > 0) {
        [[maybe_unused]] auto wsize = write(rand_fd(), &temp, sizeof(uint64_t));
    }
}

int IOMgrTester::rand_fd() {
    return m_ev_fd[std::rand() % num_ev_fd];
}
...
void IOMgrTester::stop() {
    m_iomgr->stop();
}

void 
IOMgrTester::process_ev_callback(const int fd, const void* cookie __attribute__((unused)), const int event) {
...
void IOMgrTester::set_ev_tri_type(EvtTriggerType t) {
    m_ev_tri_type = t;
}
...
uint64_t IOMgrTester::get_cb_cnt() const {
    return m_cb_cnt;
}
