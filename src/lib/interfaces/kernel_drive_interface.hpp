...
public:
    ...
    virtual std::error_code sync_read(IODevice* iodev, char* data, uint32_t size, uint64_t offset) override;
    virtual std::error_code sync_readv(IODevice* iodev, const iovec* iov, int iovcnt, uint32_t size,
                                       uint64_t offset) override;
    virtual std::error_code sync_write_zero(IODevice* iodev, uint64_t size, uint64_t offset) override;
...
private:
    std::error_code write_zero_ioctl(const IODevice* iodev, uint64_t size, uint64_t offset);
    std::error_code write_zero_writev(IODevice* iodev, uint64_t size, uint64_t offset);
