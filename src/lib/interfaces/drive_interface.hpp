...
#include <iomgr/iomgr_types.hpp> // drive_type, drive_interface_type
#include <iomgr/drive.hpp>       // drive_attributes (public value type)

#include "io_interface.hpp" // IOInterface (internal)
...
public:
    ...
    // The io_uring scheduler defers + batches submissions in poll_once, so there is no explicit
    // submit_batch()/part_of_batch any more.
    virtual exec::task< std::error_code > async_write(IODevice* iodev, const char* data, uint32_t size,
                                                      uint64_t offset) = 0;
    virtual exec::task< std::error_code > async_writev(IODevice* iodev, const iovec* iov, int iovcnt, uint32_t size,
                                                       uint64_t offset) = 0;
    virtual exec::task< std::error_code > async_read(IODevice* iodev, char* data, uint32_t size, uint64_t offset) = 0;
    virtual exec::task< std::error_code > async_readv(IODevice* iodev, const iovec* iov, int iovcnt, uint32_t size,
                                                      uint64_t offset) = 0;
    virtual exec::task< std::error_code > async_unmap(IODevice* iodev, uint32_t size, uint64_t offset) = 0;
