...
struct ioMgrImpl : public std::enable_shared_from_this< ioMgrImpl > {
    ...
    void remove_local_fd(int fd);
    void add_local_fd(int fd, ev_callback cb, int ev, int pri, void* cookie);
    void add_fd_to_thread(thread_info& t_info, int fd, ev_callback cb, int ev, int pri, void* cookie);
    void remove_fd_from_thread(thread_info& t_info, int fd);
